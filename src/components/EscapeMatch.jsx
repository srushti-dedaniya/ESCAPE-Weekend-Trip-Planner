...
export default function EscapeMatch({ selectedMood }) {
  ...
  // Dynamic alt text based on selected mood
  const getImageAlt = () => {
    if (selectedMood?.id === 'reset') {
    ...
    } else if (selectedMood?.id === 'alive') {
      return 'Rushing whitewater rapids cutting through a steep river gorge with adventurous kayakers navigating turbulent waters';
    } else if (selectedMood?.id === 'connect') {
      return 'Intimate beach cove at sunset with lantern-lit private shacks, fireside dining, and gentle waves lapping at the shore';
    } else if (selectedMood?.id === 'flow') {
      return 'Dramatic cliffside coastline in South Goa with infinity pool overlooking the Arabian Sea, bathed in golden hour light';
    } else if (selectedMood?.id === 'disappear') {
      return 'Starry night sky over Bhandardara lake with dense forest silhouettes and stargazer domes reflecting in still water';
    }
    return 'Scenic destination view';
  };

  const getImageSrc = () => {
    if (selectedMood?.id === 'reset') {
      return 'https://lh3.googleusercontent.com/aida-public/AB6AXuDrP9DarRCB2x9o-qN_naf2nSiFR3p4sT7xJAhuOmypYIuNYSTHHBjNqOLPk3zCZAJadPOG9d2HP9p8erN6hI2pqPGSl06KX0WwhN0faKmNRnpk79v0ArNZGKjqb9HRrhhjAfMv9SR4HvcfArpDGMJrIfLQopyTFFAPxKQZwbK10OvNT971Cqlw_qrWcaiyqv19UZmRhi00BaiB8YvSVTnLFD0Mqa7DMgp1U4amgX2-uveh5EDlHb5Nqw';
    } else if (selectedMood?.id === 'alive') {
      return 'https://lh3.googleusercontent.com/aida-public/AB6AXuDYKGcueE7ElrjarpcWv3aN8SY0wlp2V8iqnq-gb-camvTh5ZZePVcQjUo1lSibtrqj_V2xirnLbc1mOtL7bW5kjYc2i3pxrRZoYh7SauN1gLxAiG2MZNYzomXiB_UwYEHzcb1aMD49sHrkv2w8VXMSR-oyRaPQkV4Y7-jhTznA7FZRK3558r_ZaHf-1fAWH5bimWjN_L2fsC34k6102EFwL-F-bCQnVJU4Y5ZGG9eJV_gQWOGjIdkXXw';
    } else if (selectedMood?.id === 'connect') {
      return 'https://lh3.googleusercontent.com/aida-public/AB6AXuDYKGcueE7ElrjarpcWv3aN8SY0wlp2V8iqnq-gb-camvTh5ZZePVcQjUo1lSibtrqj_V2xirnLbc1mOtL7bW5kjYc2i3pxrRZoYh7SauN1gLxAiG2MZNYzomXiB_UwYEHzcb1aMD49sHrkv2w8VXMSR-oyRaPQkV4Y7-jhTznA7FZRK3558r_ZaHf-1fAWH5bimWjN_L2fsC34k6102EFwL-F-bCQnVJU4Y5ZGG9eJV_gQWOGjIdkXXw';
    } else if (selectedMood?.id === 'flow') {
      return 'https://lh3.googleusercontent.com/aida-public/AB6AXuDYKGcueE7ElrjarpcWv3aN8SY0wlp2V8iqnq-gb-camvTh5ZZePVcQjUo1lSibtrqj_V2xirnLbc1mOtL7bW5kjYc2i3pxrRZoYh7SauN1gLxAiG2MZNYzomXiB_UwYEHzcb1aMD49sHrkv2w8VXMSR-oyRaPQkV4Y7-jhTznA7FZRK3558r_ZaHf-1fAWH5bimWjN_L2fsC34k6102EFwL-F-bCQnVJU4Y5ZGG9eJV_gQWOGjIdkXXw';
  ...
  return (
    <section className="py-space-2xl px-gutter-mobile lg:px-gutter-desktop max-w-7xl mx-auto w-full" aria-labelledby="match-heading">
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-space-sm mb-space-lg">
        <div className="flex flex-col gap-space-2xs">
          <div className="flex items-center gap-space-xs" aria-label="AI confidence indicator">
            <span className="w-2 h-2 rounded-full bg-primary-container shadow-[0_0_8px_#ff6b4a]" aria-hidden="true"></span>
          ...
          <h2 id="match-heading" className="font-headline-lg text-headline-lg uppercase text-on-surface">Your Curated Match</h2>
        </div>
        <span className="font-label-numeric text-label-numeric text-on-surface-variant" aria-hidden="true">MATCH CRITERIA: SILENCE // ACCESSIBILITY // LUXE</span>
      </div>
      ...
      <motion.div 
        ...
        role="region"
        aria-label="Escape match details"
      >
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent" aria-hidden="true"></div>

        <div className="lg:col-span-5 flex flex-col gap-space-lg">
          <div className="flex items-center gap-space-lg">
            <motion.div className="relative w-28 h-28 flex-shrink-0 flex items-center justify-center" aria-label={`${selectedMood?.score || 94}% vibe fit match score`}>
              <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 100 100" aria-hidden="true" focusable="false">
            ...
            <div className="flex flex-col gap-space-2xs">
              ...
              >
                <span className="material-symbols-outlined text-[16px] text-secondary" aria-hidden="true">near_me</span> {selectedMood?.matchDistance || '2.5 hours from Mumbai'}
              </motion.span>
            </div>
          </div>
          ...
          <motion.div 
            ...
            role="list"
            aria-label="Destination features"
          >
            <div className="p-space-sm rounded-xl bg-surface-container-high/60 flex items-start gap-space-xs" role="listitem">
              <span className="material-symbols-outlined text-secondary text-[20px] mt-0.5" aria-hidden="true">park</span>
              <div>
            ...
            <div className="p-space-sm rounded-xl bg-surface-container-high/60 flex items-start gap-space-xs" role="listitem">
              <span className="material-symbols-outlined text-primary text-[20px] mt-0.5" aria-hidden="true">timer</span>
              <div>
            ...
            <div className="p-space-sm rounded-xl bg-surface-container-high/60 flex items-start gap-space-xs" role="listitem">
              <span className="material-symbols-outlined text-tertiary text-[20px] mt-0.5" aria-hidden="true">payments</span>
              <div>
            ...
            <div className="p-space-sm rounded-xl bg-surface-container-high/60 flex items-start gap-space-xs" role="listitem">
              <span className="material-symbols-outlined text-secondary text-[20px] mt-0.5" aria-hidden="true">volume_off</span>
              <div>
          ...
          >
            <a 
              className="inline-flex items-center gap-space-xs px-space-xl py-space-sm rounded-full bg-primary-container text-on-primary-container font-label-caps text-label-caps uppercase tracking-wider font-bold shadow-[0_0_20px_rgba(255,107,74,0.35)] hover:bg-primary transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-container focus-visible:ring-offset-2 focus-visible:ring-offset-surface" 
              href="#itinerary"
            >
              Unlock Itinerary <span className="material-symbols-outlined text-[16px]" aria-hidden="true">arrow_forward</span>
            </a>
        ...
        >
          <img 
            ...
            src={getImageSrc()}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-surface-container-lowest via-surface-container-lowest/30 to-transparent" aria-hidden="true"></div>
          <div className="absolute top-4 left-4 flex gap-space-2xs" aria-label="Location info">
            <span className="px-space-sm py-space-2xs rounded-full bg-surface-container-lowest/80 backdrop-blur-md font-label-caps text-label-caps uppercase text-secondary">
              {selectedMood?.id === 'reset' ? 'MATHERAN • 800M ELEVATION' : 
               selectedMood?.id === 'alive' ? 'KUNDALIKA GORGE • 3.0 HRS AWAY' :
               selectedMood?.id === 'connect' ? 'KASHID SECRET COVE • 3.2 HRS AWAY' :
               selectedMood?.id === 'flow' ? 'GOA CLIFFS • 50 MIN FLIGHT' :
               'BHANDARDARA GLADES • 3.8 HRS AWAY'}
            </span>
