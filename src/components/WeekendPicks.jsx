...
const picks = [
  {
    ...
    alt: 'Matheran deep canopy forest with misty trails, red soil paths, and toy train ascending through dense greenery'
  },
  {
    id: 2,
    ...
    alt: 'Alibaug coastal cliffs at sunset with private beach access and cliffside walkways'
  },
  {
    id: 3,
    ...
    alt: 'Lonavala twin lakes surrounded by Sahyadri mountains with waterfall trails and ancient caves'
  },
  {
    id: 4,
    ...
    alt: 'Mahabaleshwar ridge with strawberry plateaus, colonial bungalows, and 360-degree valley views in fog'
  },
  {
    id: 5,
    ...
    alt: 'Kamshet paragliding basin with colorful paragliders soaring over lake and thermal currents at sunrise'
  }
];

export default function WeekendPicks() {
  return (
    <section id="weekend-picks" className="py-space-4xl px-gutter-mobile lg:px-gutter-desktop max-w-7xl mx-auto w-full" aria-labelledby="picks-heading">
      <motion.div 
      ...
      >
        <div className="flex flex-col gap-space-2xs">
          ...
          <h2 id="picks-heading" className="font-headline-lg text-headline-lg uppercase text-on-surface">Curated Weekend Picks</h2>
        ...
        <div className="flex items-center gap-space-xs font-label-caps text-label-caps uppercase text-on-surface-variant" aria-hidden="true">
      ...
      <motion.div 
        ...
        role="list"
        aria-label="Weekend destination picks"
      >
        {picks.map((pick) => (
          <motion.article
            key={pick.id}
          ...
          >
            <div 
              ...
              aria-label={pick.alt}
            ></div>
            <div className="absolute inset-0 bg-gradient-to-t from-surface-container-lowest/95 via-surface-container-lowest/10 to-transparent z-10" aria-hidden="true"></div>
            ...
            <div className="relative z-20 flex flex-col justify-end p-space-lg lg:p-space-xl gap-space-md min-h-[380px] lg:min-h-[460px]">
              ...
              <div className="flex flex-wrap gap-space-sm" role="list" aria-label={`${pick.name} highlights`}>
                {pick.highlights.map((highlight, i) => (
                  <span key={i} className="px-space-sm py-1 rounded-full bg-surface-container-lowest/80 backdrop-blur-md font-label-caps text-[10px] uppercase text-on-surface-variant/90 border border-outline/30" role="listitem">{highlight}</span>
                ))}
              </div>
              ...
              <motion.a
                className="inline-flex items-center gap-space-xs px-space-md py-space-sm rounded-full bg-primary-container text-on-primary-container font-label-caps text-label-caps uppercase tracking-wider font-bold shadow-[0_0_20px_rgba(255,107,74,0.35)] hover:bg-primary transition-all w-fit focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-container focus-visible:ring-offset-2 focus-visible:ring-offset-surface"
                whileHover={{ x: 4 }}
                href="#mood-section"
              >
                View Manifest <span className="material-symbols-outlined text-[16px]" aria-hidden="true">arrow_forward</span>
              </motion.a>
            </div>
          </motion.article>
        ))}
