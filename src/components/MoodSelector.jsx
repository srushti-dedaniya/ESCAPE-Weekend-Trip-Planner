...
export default function MoodSelector({ selectedMood, onMoodSelect }) {
  return (
    <section className="relative py-space-4xl px-gutter-mobile lg:px-gutter-desktop max-w-7xl mx-auto w-full" id="mood-section">
      <motion.div 
      ...
      >
        ...
        <h2 className="font-headline-lg text-headline-lg lg:text-[48px] uppercase tracking-tight text-on-surface">How do you want to feel?</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-space-md relative z-10" id="mood-pods-container">
          ...
          <motion.button
            ...
            className={`mood-pod text-left group p-space-lg rounded-2xl backdrop-blur-2xl shadow-xl transition-all duration-500 flex flex-col justify-between min-h-[220px] relative overflow-hidden ${
              selectedMood?.id === mood.id
                ? 'bg-surface-container-high/80 shadow-[0_20px_40px_rgba(0,0,0,0.4)]'
                : 'bg-surface-container/60 hover:bg-surface-container-high'
            }`}
            data-dist={mood.distance}
            data-mood={mood.id}
            data-name={mood.destination}
